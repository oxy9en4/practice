...
bool Sample::Init()
{
	const Texture* pTex = I_Tex.Load(L"../../res/ui/0.png");
...
bool Sample::Render()
{
	m_pMapObj->SetMatrix(nullptr, &m_MainCamera.m_matView, &m_MainCamera.m_matOrthoProj);
	m_pMapObj->Render();

	m_pSpriteObj->SetMatrix(nullptr, &m_MainCamera.m_matView, &m_MainCamera.m_matOrthoProj);
